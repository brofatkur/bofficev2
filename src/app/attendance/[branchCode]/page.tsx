  ...
  return (
    ...
    >
      ...
      >
        ...
        {activeLog ? (
          <div className="bg-gradient-to-b from-emerald-50/90 to-white rounded-2xl border border-emerald-200 p-6 text-center space-y-4 shadow-sm">
            <div className="flex items-center justify-center gap-2 text-xs font-semibold text-emerald-800 bg-emerald-100 py-1 px-3 rounded-full border border-emerald-200 w-fit mx-auto">
            ...
            <div>
              <div className="text-slate-500 text-xs">Waktu Berjalan (Stopwatch):</div>
              <div className="text-4xl font-black font-mono text-emerald-600 tracking-wider mt-1 animate-pulse">
                {formatStopwatch(elapsedSeconds)}
              </div>
            </div>

            <div className="pt-2 border-t border-slate-100 text-xs text-left space-y-1.5 font-sans">
              <div className="flex justify-between">
                <span className="text-slate-500">Penanggung Jawab:</span>
                <span className="font-bold text-slate-900">{activeLog.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Lembaga / Tenant:</span>
                <span className="font-semibold text-blue-600">{activeLog.organization}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Waktu Masuk (Check-In):</span>
            ...
            <button
              ...
              className="w-full py-3.5 bg-rose-600 hover:bg-rose-700 text-white font-extrabold rounded-2xl shadow-lg shadow-rose-600/20 transition-all flex items-center justify-center gap-2 text-sm disabled:opacity-50"
            >
          ...
          <div className="space-y-4">
            <form onSubmit={handleSearchBooking} className="space-y-3">
              <div>
                <label className="block font-semibold text-slate-700 text-xs mb-1 flex items-center gap-1.5">
                  <Phone className="w-3.5 h-3.5 text-blue-600" /> Masukkan No. WhatsApp / HP Pemesan *
                </label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-slate-50 border border-slate-300 rounded-xl text-slate-900 font-mono text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all"
                  />
                  <button
                    type="submit"
                    disabled={searching}
                    className="px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl transition-all shadow-md shadow-blue-600/20 flex items-center gap-1.5 disabled:opacity-50 shrink-0"
                  >
                    {searching ? (
            ...
            {searchAttempted && bookingFound && bookingData && (
              <div className="bg-emerald-50/80 p-5 rounded-2xl border border-emerald-200 space-y-4 animate-fadeIn text-xs">
                ...
                <div className="space-y-2 text-slate-700">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-500">Lembaga / Tenant:</span>
                    <span className="font-bold text-blue-600">{bookingData.tenantName}</span>
                  </div>
                  <div className="flex justify-between">
            ...
            {searchAttempted && !bookingFound && (
              <div className="bg-amber-50/90 p-5 rounded-2xl border border-amber-200 space-y-4 animate-fadeIn text-xs text-center">
                <div className="w-12 h-12 bg-amber-100 text-amber-600 rounded-2xl flex items-center justify-center mx-auto shadow-sm">
                  <CalendarX className="w-6 h-6" />
                </div>

                <div>
                  <h3 className="font-bold text-slate-900 text-sm">Belum Ada Data Booking</h3>
                  <p className="text-slate-600 text-xs mt-1 leading-relaxed">
                    Nomor HP <strong className="text-slate-900 font-mono">{phone}</strong> belum memiliki jadwal booking terkonfirmasi di cabang ini.
                ...
                <Link
                  ...
                  className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-md shadow-blue-600/20 transition-all text-xs flex items-center justify-center gap-2"
                >
      ...
      {receipt && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl max-w-sm w-full p-6 text-center space-y-4 shadow-2xl animate-fadeIn text-slate-800">
            ...
            <div>
              <h3 className="font-extrabold text-slate-900 text-base">Sesi Selesai</h3>
              <p className="text-slate-500 text-xs mt-0.5">Bukti Pemakaian Ruang Rapat BOffice</p>
            </div>

            <div className="bg-slate-50 p-4 rounded-2xl text-xs space-y-2 text-left border border-slate-200 font-sans shadow-sm">
              <div className="flex justify-between">
                <span className="text-slate-500">Penanggung Jawab:</span>
                <span className="font-bold text-slate-900">{receipt.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Lembaga / Tenant:</span>
                <span className="text-blue-600 font-semibold">{receipt.organization}</span>
              </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-700 font-bold rounded-xl text-xs transition-all"
            >
              Tutup
            </button>
          </div>
